...
public class TicTacToeView {
   ...
   public void printOccupiedPositionInfo(byte level, byte row, byte column) {
      printStream.printf("ERROR: Cell in level = %d  row = %d  column = %d is not empty %n", level, row, column);
   }
   ...
   public void printMoveCoordinationsInfo(String playerName, byte level, byte row, byte column) {
      ...
      printStream.printf(" %s marks cell in level = %d  row = %d  column = %d %n", playerName, level, row, column);
   }
   ...
   public void printBoard(TicTacToe ticTacToe) {
      ...
      String boardString = generateBoardString(ticTacToe);
      printStream.println(boardString);
   } 
   
   private String generateBoardString(TicTacToe ticTacToe) {
      final CellValue[][][] BOARD = ticTacToe.getBoard();
      CellValue[][] boardOnLevel;
      String boardString = "";
      
      for (byte level = 0; level < TicTacToe.getSQUARE_SIZE(); ) {
         boardOnLevel = BOARD[level];
         
         boardString += String.format("%n \t LEVEL %d %n", ++level);
         
         boardString += generateBoardStringByLevel(boardOnLevel);
      }
      
      return boardString;
   }
   
   private String generateBoardStringByLevel(CellValue[][] BOARD) {
      String boardString = NUMBERS_ROW;
